	...
	(org-capture nil "n"))
      ...
      (setq org-capture-templates
            '(("b" "Web Bookmark" entry (file+headline "~/.org/refile.org" "Bookmarks") "* %c\n:PROPERTIES:\n:CREATED: %U\n:END:\n%i")
              ...
              ("jj" "Journal Entry" plain (file+datetree "~/.org/journal.org") (file "~/.org/templates/review"))
              ("ja" "Journal Entry" entry (file+datetree "~/.org/journal.org") "* %u Accomplished: %?")
              ("jn" "Journal Entry" entry (file+datetree "~/.org/journal.org") "* %u %?")
...
- creation of new issues from headlines the format of:

#+BEGIN_SRC text
\* TODO # - Title of the issue                                 :@bug:
#+END_SRC
...
(defun leoc/org-redmine-sync (&optional prefix)
  ...
  (save-some-buffers)
  (if prefix
      (let ((from-timestamp (format-time-string "%Y-%m-%d" (time-subtract (current-time) (seconds-to-time (* 2 24 60 60)))))
	    (to-timestamp (format-time-string "%Y-%m-%d" (current-time))))
	(shell-command (concat "urxvt -e bash --login -c \"cd /home/arthur/projects/org-redmine-time-transfer; ./org-redmine sync" from-timestamp "--" to-timestamp ";\"") nil nil))
